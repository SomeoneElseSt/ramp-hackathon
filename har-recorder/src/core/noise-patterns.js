...
// Tuned against real LinkedIn activity traces (Downloads) + Unbrowse patterns.
// Drop before scoring / listener discovery / always-on daemon ingest.

/** Hosts that are pure noise — CDN, tracking, ads, fonts. */
...
/**
 * Session / LinkedIn plumbing — looks like /api/ but is not listenable product data.
 * KEEP: messengerMessages, messengerConversations, realtime/connect.
 */
...
export function isNoiseUrl(url) {
  ...
  if (url.startsWith("data:") || url.startsWith("blob:") || url === "invalid") return true;
