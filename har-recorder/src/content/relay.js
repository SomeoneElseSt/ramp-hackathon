...
// Idempotent: content_scripts (document_start) + programmatic inject may both run.
(() => {
  if (window.__wfRelay) return;
  window.__wfRelay = true;
  let active = true;
  window.addEventListener("message", (e) => {
    ...
    try {
      chrome.runtime.sendMessage({ type: "page-capture", rec: d.rec });
    } catch (_) {}
  });
